...
template<class T>
inline bool StructuredBuffer<T>::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext)
{
	std::vector<T>tmp;
	T r;
	tmp.emplace_back(r);
	return Initialize(device, deviceContext, tmp);
}
...
template<class T>
inline int StructuredBuffer<T>::getSizeOfVector() const
{
